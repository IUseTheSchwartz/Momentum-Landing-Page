...
export default function Reschedule() {
  ...
  async function verify() {
    try {
      setErr("");
      setLoading(true);
      // Call your Netlify function to validate and fetch current appt + open slots
      const res = await fetch("/.netlify/functions/reschedule", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "lookup", appt_id: appt, token, email }),
      });
      const json = await res.json();
      if (!res.ok || !json?.ok) throw new Error(json?.error || "Could not verify.");
  ...
  async function doReschedule(startUtc) {
    try {
      ...
      const res = await fetch("/.netlify/functions/reschedule", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "reschedule", appt_id: appt, token, start_utc: startUtc }),
      });
      const json = await res.json();
  ...
  useEffect(() => {
    // basic guardrail
    if (!appt || !token) setErr("Invalid reschedule link.");
  }, [appt, token]);
    ...
    <div className="min-h-screen bg-[#1e1f22] text-white p-6">
      <div className="max-w-xl mx-auto">
        ...
        {!err ? (
            ...
            {step === "verify" && (
              <div className="space-y-3">
                <p className="text-white/70">
                  Enter the email you used to book so we can confirm your appointment.
                </p>
                <input
                  type="email"
          ...
          <div className="rounded-lg border border-red-400/30 bg-red-400/10 p-3">{err}</div>
        )}
      </div>
    </div>
  );
