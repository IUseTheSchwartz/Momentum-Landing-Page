const { formatAppt } = require("./_time");

// Very light CSS inline for deliverability.
...
function clientConfirm({ whenIso, tz, durationMin = 30, rescheduleUrl, phoneHref, phoneLabel, vcardUrl }) {
...
function agentApptNotice({ lead, whenIso, tz, durationMin = 30, adminUrl }) {
  const when = formatAppt(whenIso, tz);
  const html = `
...
function incompleteNotice({ lead, resumeUrl, lastActiveRel }) {
  const html = `
...
module.exports = { clientConfirm, agentApptNotice, incompleteNotice };
