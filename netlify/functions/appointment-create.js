const { getServiceClient } = require("./_supabase"); // your existing helper
...
exports.handler = async (event) => {
  ...
  try {
    ...
    // Email client (if we have their email)
    if (lead.email) {
    ...
    // Log email (de-dupe basis)
    await supabase.from("mf_email_log").insert([
      ...(lead.email ? [{ lead_id, type: "appointment" }] : []),
    ]);
